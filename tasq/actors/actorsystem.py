...
class ActorSystem(metaclass=SingletonArgs):
    ...
    def actor_of(self, actorclass, name="", *args, **kwargs):
        """Return the reference of an actor based on the actor class passed"""
        actor = actorclass(*args, name=name, ctx=self, **kwargs)
        self._actors[actor.name] = actor
        return actor

    def router_of(
        self, router_class, actor_class, num_workers, func_name="submit", *args, **kwargs
    ):
        """Return a router class type with the number and type of workers
        specified
        """
        pool = actor_pool(num_workers, actor_class, router_class, func_name, *args, **kwargs)
