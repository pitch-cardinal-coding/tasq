...
class RoundRobinRouter(Router):
    ...
    def __init__(self, workers, func_name):
        super().__init__(workers, func_name)
        self._idx = 0
...
def actor_pool(num_workers, actor_class, router_class, func_name="submit", *args, **kwargs):
    ...
    if clients:
        return router_class(
            [
                actor_class(*args, name=f"{actorname}-{i}", client=c, **kwargs)
                for i, c in zip(range(num_workers), clients)
            ],
            func_name,
        )
    return router_class(
        [actor_class(*args, name=f"{actorname}-{i}", **kwargs) for i in range(num_workers)],
        func_name,
    )
