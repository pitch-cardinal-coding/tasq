...
class Client:
    ...
    def _gather_results(self):
        ...
        while not self._gather_loop.is_set():
            try:
                ...
                self._log.warning("Backend error while receiving results back: %s", str(e))
            else:
                if not job_result:
                    continue
    ...
    def schedule_blocking(self, func, *args, **kwargs):
        ...
        """
        if not self.is_connected():
            raise ClientNotConnectedException("Client not connected to no worker")
